#!/usr/bin/env python3
"""
Tests for the unified GUI tool.
...
# Add the parent directory to the path to import the unified_gui module
sys.path.insert(0, str(Path(__file__).parent.parent))

from unified_gui import UnifiedBookTool
...
class TestUnifiedBookTool(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch('unified_gui.ttk'), \
             patch('unified_gui.tk'), \
             patch('unified_gui.messagebox'), \
             patch('unified_gui.filedialog'):
            self.tool = UnifiedBookTool(self.root)
            
    def test_initialization(self):
        """Test that the tool initializes correctly."""
        self.assertIsNotNone(self.tool.capture_handler)
        self.assertEqual(self.tool.default_output_folder, str(Path.cwd() / "captured_pages"))
    ...
    def test_validate_inputs_valid(self):
        ...
        with patch('unified_gui.messagebox'):
    ...
    def test_validate_inputs_invalid_capture_params(self):
        ...
        with patch('unified_gui.messagebox') as mock_messagebox:
            result = self.tool.validate_inputs()
    ...
    @patch('unified_gui.subprocess.run')
    def test_take_test_screenshot_success(self, mock_subprocess):
        ...
        with patch('unified_gui.Path') as mock_path:
            mock_temp_path = Mock()
            mock_path.cwd.return_value.__truediv__.return_value = mock_temp_path
            ...
            mock_subprocess.assert_called_once_with(
                ['import', '-window', 'root', str(mock_temp_path)],
                check=True, capture_output=True
            )
            
            # Verify the preview was loaded
            self.assertEqual(self.tool.current_preview_path, mock_temp_path)
